...
from core.layers.RegionAttentionBlock import RAB

class RAPCell(nn.Module):
    def __init__(self, in_channel, num_hidden, width, filter_size, stride, patch_size, layer_norm):
        super(RAPCell, self).__init__()
        ...
        self.input_rab_layer = RAB(
            class_num=64,
            channels=self.in_channel,
            width=width,
            inner_dim=512,
        )
        self.hidden_rab_layer = RAB(
            class_num=64,
            channels=self.num_hidden,
            width=width,
            inner_dim=512,
        )
    ...
    def forward(self, x_t, h_t, c_t, m_t):
        ...
        x_concat = self.input_rab_layer(x_t)
        x_concat = self.conv_x(x_concat)
        h_concat = self.hidden_rab_layer(h_t)
        h_concat = self.conv_h(h_concat)
